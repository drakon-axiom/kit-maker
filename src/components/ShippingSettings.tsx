...
import { Loader2, Save, Plus, Pencil, Trash2, Package, Truck } from 'lucide-react';
  ...
  const fetchData = useCallback(async () => {
    try {
      const [settingsRes, presetsRes] = await Promise.all([
        ...
        supabase.from('box_presets').select('*').order('name'),
      ]);
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="shipstation_carrier_code">Default Carrier Code</Label>
              <Input
                id="shipstation_carrier_code"
                value={settings.shipstation_carrier_code || ''}
                onChange={(e) => setSettings({ ...settings, shipstation_carrier_code: e.target.value })}
            ...
            <div className="space-y-2">
              <Label htmlFor="shipstation_service_code">Default Service Code</Label>
              <Input
                id="shipstation_service_code"
                value={settings.shipstation_service_code || ''}
                onChange={(e) => setSettings({ ...settings, shipstation_service_code: e.target.value })}
                placeholder="e.g., usps_priority_mail"
              />
              <p className="text-xs text-muted-foreground">
                Example: usps_priority_mail, fedex_ground, ups_ground
              </p>
            </div>
          </div>
          <Button
            onClick={async () => {
              await handleSaveSetting('shipstation_carrier_code', settings.shipstation_carrier_code || '');
              await handleSaveSetting('shipstation_service_code', settings.shipstation_service_code || '');
            }}
            disabled={saving}
            size="sm"
          >
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Saved box dimensions for quick selection when creating labels</CardDescription>
          </div>
              ...
              <div className="space-y-4 pt-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Optional - can override per shipment"
                  />
                </div>
                <div className="flex items-center gap-2">
                  ...
                  />
                  <Label htmlFor="preset-default">Set as default preset</Label>
                </div>
        ...
        <CardContent>
          {boxPresets.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">No box presets configured</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {boxPresets.map((preset) => (
                  <TableRow key={preset.id}>
                    ...
                    <TableCell>{preset.weight_oz ? `${preset.weight_oz} oz` : '-'}</TableCell>
                    <TableCell>{preset.is_default ? '✓' : ''}</TableCell>
                    <TableCell>
                      <Switch
